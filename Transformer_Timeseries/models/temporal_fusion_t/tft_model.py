...
class TFT(BaseModel):
    def __init__(self, raw_params):
        ...
        self.num_locations = int(params.get('num_locations', params.get('location_vocab_size', 256)))
        if len(self.category_counts) > 0 and self.num_locations > int(self.category_counts[0]):
            self.category_counts[0] = self.num_locations
        self.n_multiprocessing_workers = int(params['n_workers'])
        self.use_location_token = bool(params.get('use_location_token', True))
        self.location_input_idx = int(params.get('location_input_idx', self.input_size - 1))
        ...
        if self.use_location_token:
            self.location_token_embedding = nn.Embedding(max(2, self.num_locations), self.hidden_layer_size)
        else:
            self.location_token_embedding = None
    ...
    def forward(self, x):
        ...
        location_token = None
        if self.use_location_token and self.location_token_embedding is not None:
            loc_ids = all_inputs[:, 0, self.location_input_idx].long()
            loc_ids = torch.clamp(loc_ids, 0, self.location_token_embedding.num_embeddings - 1)
            location_token = self.location_token_embedding(loc_ids).unsqueeze(1)
            all_inputs = all_inputs.clone()
            all_inputs[:, :, self.location_input_idx] = 0.0
        ...
        if location_token is not None:
            temporal_feature_layer = torch.cat([location_token, temporal_feature_layer], dim=1)
        ...
        start_idx = self.num_encoder_steps + (1 if location_token is not None else 0)
        outputs = self.output_layer(transformer_layer[:, start_idx:, :])
