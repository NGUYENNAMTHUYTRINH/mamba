...
class Transformer(nn.Module):
    ...
    def __init__(self, cnf: dict):
        ...
        self.use_location_token = bool(cnf.get("use_location_token", False))
        self.location_input_idx = int(cnf.get("location_input_idx", d_input - 1))
        num_locations = int(cnf.get("num_locations", cnf.get("location_vocab_size", 1024)))
        ...
        if self.use_location_token:
            self._location_embedding = nn.Embedding(max(2, num_locations), d_model)
        else:
            self._location_embedding = None
    ...
    def forward(self, xy: torch.Tensor) -> torch.Tensor:
        ...
        """
        if self.use_location_token:
            # Get location id from configured input column (kept constant within each window)
            loc_ids = xy[:, 0, self.location_input_idx].long()
            loc_ids = torch.clamp(loc_ids, 0, self._location_embedding.num_embeddings - 1)
            loc_token = self._location_embedding(loc_ids).unsqueeze(1)

            # Avoid feeding raw location-id magnitude directly to linear projection
            xy = xy.clone()
            xy[:, :, self.location_input_idx] = 0.0
        else:
            loc_token = None
        ...
        if loc_token is not None:
            # Mamba-like location token prepended to sequence representation
            encoding_x = torch.cat((loc_token, encoding_x), dim=1)

        # Add position encoding
        if self._generate_PE is not None:
            positional_encoding = self._generate_PE(encoding_x.shape[1], self._d_model)
