...
def unnormalize_tensor(data_formatter, data, identifier):
    ...
    # Kiểm tra xem data có phải là Pandas DataFrame (có hàm drop) không
    if hasattr(data, 'drop'):
        return data.drop(columns=['identifier']).values
    else:
        # Nếu là NumPy array, cột identifier thường là cột cuối cùng được thêm vào.
        # Ta dùng cú pháp slicing của NumPy để bỏ đi cột cuối cùng.
